...
export function fabricItemFromDoc(id: string, d: DocumentData): FabricCatalogItem {
  ...
  return {
    ...
    pricePerMeter: num(d.pricePerMeter),
    weightPercent: num(d.weightPercent),
    softnessPercent: num(d.softnessPercent),
    gsm: String(d.gsm ?? ""),
    region: String(d.region ?? ""),
    availableMeters: num(d.availableMeters),
...
export function parseFabricInput(body: unknown): { ok: true; value: FabricCatalogInput } | { ok: false; error: string } {
  ...
  return {
    ...
    value: {
      ...
      pricePerMeter: num(b.pricePerMeter),
      weightPercent: num(b.weightPercent),
      softnessPercent: num(b.softnessPercent),
      gsm: String(b.gsm ?? ""),
      region: String(b.region ?? ""),
      availableMeters: num(b.availableMeters),
