...
export function SignupStep2Form() {
  const { step1, step2, prevStep, setCurrentStep } = useSignupStore();
  ...
  const form = useForm<Step2Values>({
    ...
    initialValues: {
      taxId: step2.taxId,
      expectedYardage: step2.expectedYardage,
      preferredFabricType: step2.preferredFabricType,
      brandVision: step2.brandVision,
    },
    validate: {
      taxId: (value) => (value.trim() ? null : ""),
  ...
  const handleSubmit = form.onSubmit(async (values) => {
    ...
    try {
      ...
      // Show step-3 success state in the current signup flow
      setCurrentStep(3);
    } catch (err) {
      ...
      <Box component="form" onSubmit={handleSubmit} className="flex flex-col gap-5 px-10 pb-8 pt-4">
        ...
        <Box className="group/input relative">
          <TextInput
            ...
            placeholder="e.g. 500-1000 yards"
            autoComplete="off"
            required
